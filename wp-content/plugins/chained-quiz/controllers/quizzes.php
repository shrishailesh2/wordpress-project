		...
		if(!empty($_SESSION['chained_completion_id'])) {
			...
			if($exists) {
				$wpdb->query($wpdb->prepare("UPDATE ".CHAINED_USER_ANSWERS." SET
					answer=%s, points=%f, comments=%s WHERE quiz_id=%d AND completion_id=%d AND question_id=%d", 
					$answer, $points, $comments, $quiz->id, intval($_SESSION['chained_completion_id']), $question->id));			
			}
			else {				
				$wpdb->query($wpdb->prepare("INSERT INTO ".CHAINED_USER_ANSWERS." SET
					quiz_id=%d, completion_id=%d, question_id=%d, answer=%s, points=%f, comments=%s",
					$quiz->id, intval($_SESSION['chained_completion_id']), $question->id, $answer, $points, $comments));				
			}		
